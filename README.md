...
Two scenarios of three signs each, plus four bonus games — including a two-player duel
and a strict 6-7 counter. Everything runs locally: no video is uploaded, and after the
first load the app needs no network at all.
...
src/engine/       the rAF loops: useAttempt, useSignRound, useDuelTurn, useSixSevenRound
...
src/screens/      Home, ScenarioGame, RushHour, RushDuel, AiGuess, SixSeven
```
